...
function requestedOutputMode(request: Request): OutputMode {
  return request.headers.get("x-zyron-output-mode")?.toLowerCase() === "text" ? "text" : "audio";
}

function buildRealtimeSession(outputMode: OutputMode) {
  return {
    type: "realtime",
...
"En preguntas simples y directas, responde únicamente a lo preguntado, sin añadir introducciones, muletillas, contexto, comentarios ni sugerencias no solicitadas. Empiezas directamente por la respuesta. Por ejemplo, si Aarón pregunta qué hora es. responde solo con la hora",      
"Si Aarón te interrumpe, deja de responder inmediatamente y escucha el nuevo turno.",
      outputMode === "text"
        ...
        : "Esta sesión está en modo de voz. Responde por audio de forma natural.",
...
"Nunca verbalices que vas a consultar, comprobar, buscar o usar una herramienta. Ejecuta las herramientas en silencio y responde directamente con el resultado, sin frases como «déjame comprobarlo», «voy a consultarlo» o similares.",guardado
    ...
    tools: [
      ...
      },
      {
        ...
        description: "Busca negocios y lugares físicos reales en Google Places, con nombre y dirección verificados.",
        parameters: {
          type: "object",
          properties: {
            query: {
...
export async function POST(request: Request) {
  try {
    ...
    if (!response.ok) {
      ...
      console.error("ZYRON_NATIVE_REALTIME_ERROR", response.status, safe.code, safe.message);
    ...
    return new Response(text, {
      ...
      headers: {
        ...
        "X-Zyron-Output-Mode": outputMode,
      },
    });
  } catch (error) {
    const detail = error instanceof Error && error.message === "openai_api_key_missing"
      ? "Falta OPENAI_API_KEY en el servidor."
      : "No se ha podido crear la sesión Realtime nativa.";
    console.error("ZYRON_NATIVE_REALTIME_ERROR", error instanceof Error ? error.message : "unknown");
    return Response.json({ error: "realtime_call_failed", code: "zyron_native_realtime_server_error", detail }, { status: 500 });
  }
}
